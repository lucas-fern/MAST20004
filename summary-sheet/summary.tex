...
$\mathbb{E}(X|Y = y) \coloneqq \int_{-\infty}^{\infty}x f_{X|Y}(x|y)dx$, replacing the integral for a sum over $x \in S_X$ for the discrete case.\\[2mm]
...
$$\mbox{Var}(X|Y=y) = \mathbb{E}(X^2|Y=y) - \mathbb{E}(X^2|Y=y)^2.$$
